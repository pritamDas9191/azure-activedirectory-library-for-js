...
var AuthenticationContext = (function () {
    ...
    window.Logging = {
        ...
        log: function (message) {
            if (!window.sdBacklog) {
                window.sdBacklog = [];
                window.sdLoggerMessageCount = 0;
            }

            // Stop logging after 50 messages
            if (window.sdLoggerMessageCount >= 50) {
                return;
            }
            window.sdLoggerMessageCount++;

            // See if there is a valid logger to use
            if (window.sdLogger) {
                window.sdLogger.logInfo("Adal Log: " + message);
            } else {
                // Queue messages for safe keeping until we can get the logger
                window.sdBacklog.push(message);

                if (window.angular &&
                    window.$ &&
                    window.angular.element(window.$("body")) &&
                    window.angular.element(window.$("body")).injector) {
                    window.angular.element(window.$("body")).injector().invoke(["loggingService", function(loggingService) {
                        // Once we have an instance of the logger, save it and then push all saved messages into it
                        window.sdLogger = loggingService;
                        window.sdLogger.logInfo("Adal Logging attached to loggingService, " + window.sdBacklog.length + " messages in backlog");
                        for (var i = 0; i < window.sdBacklog.length; i++) {
                            window.Logging.log(window.sdBacklog[i]);
                        }
                    }]);
                }
            }
