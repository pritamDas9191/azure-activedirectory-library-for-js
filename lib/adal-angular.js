            ...
            this.$get = ['$rootScope', '$window', '$q', '$location', '$timeout', '$injector', function ($rootScope, $window, $q, $location, $timeout, $injector) {
                ...
                var locationChangeHandler = function (event, newUrl, oldUrl) {
                    ...
                    if (_adal.isCallback(hash)) {
                        _adal.verbose("callback can come from login or iframe request");
                        ...
                        if (requestInfo.stateMatch) {
                            if (requestInfo.requestType === _adal.REQUEST_TYPE.RENEW_TOKEN) {
                                ...
                                if (_oauthData.userName) {
                                    ...
                                    _adal.verbose("$broadcast, adal:loginSuccess, f908024b-8f3d-417a-b883-084ad80de620");
                                    $rootScope.$broadcast('adal:loginSuccess', _adal._getItem(_adal.CONSTANTS.STORAGE.IDTOKEN));
                                } else {
                                    _adal.verbose("$broadcast, adal:loginFailure, 3b93efad-caff-4be4-971d-044264a42ff1");
                                ...
                                _adal.verbose("redirect to login start page");
                                if (!_adal.popUp) {
                                    ...
                                    if (typeof loginStartPage !== 'undefined' && loginStartPage && loginStartPage.length !== 0) {
                                        ...
                                        if (!$location.$$html5 && loginStartPage.indexOf('#') > -1) {
                                            var modifiedUrl = loginStartPage.substring(loginStartPage.indexOf('#') + 1);
                                            _adal.verbose('Legacy navigation to ' + modifiedUrl);
                                            $location.url(modifiedUrl);
                                        }
                                        $window.location = loginStartPage;
                                    }
                                }
                                else {
                                    _adal.verbose("Preventing default, 5838d8e9-4149-4fee-bfa4-ab02ef68b0f2");
                                    event.preventDefault();
                                }
                            }
                        }
                            ...
                            _adal.verbose("$broadcast, adal:stateMismatch, 60ca6c8a-3b57-416a-90a9-bf03c3df2670");
                        ...
                        _adal.verbose("No callback. App resumes after closing or moving to new page.");
                        _adal.verbose("Check token and username");
                        updateDataFromCache(_adal.config.loginResource);
                        if (!_oauthData.isAuthenticated && _oauthData.userName && !_adal._renewActive) {
                            ...
                            self.acquireToken(_adal.config.loginResource).then(function (token) {
                                if (token) {
                                    _adal.verbose("acquireToken completed, isAuthenticated = true, a5302749-f145-47d9-a609-ba00143a828e");
                                    _oauthData.isAuthenticated = true;
                                }
                            }, function (error) {
                                var errorParts = error.split('|');
                                _adal.verbose("$broadcast, adal:loginFailure, " + errorParts[0] + ", " + errorParts[1] + ", bcef05da-21d5-4d80-9831-0955b56f4e6a");
                ...
                var loginHandler = function () {
                    ...
                    if (_adal.config && _adal.config.localLoginUrl) {
                        ...
                        _adal.verbose("$broadcast, adal:loginRedirect, 7aaf3935-a41f-4da7-9b0b-5b0f7bae65a3");
                ...
                return {
                    ...
                    acquireToken: function (resource) {
                        ...
                        _adal.acquireToken(resource, function (errorDesc, tokenOut, error) {
                            ...
                            if (error) {
                                _adal.verbose("$broadcast, adal:acquireTokenFailure, " + errorDesc + ", " + error + ", 0fc53ef7-7428-4b74-be0b-30e7c767d2d0");
                                ...
                                _adal.verbose("$broadcast, adal:acquireTokenSuccess, a28fb650-a300-4352-9fdb-19d6bece2258");
                ...
                request: function (config) {
                    if (config) {
                        ...
                        if (tokenStored) {
                            ...
                            _adal.verbose("Cancel request if login is starting");
                            if (authService.loginInProgress()) {
                                ...
                                _adal.verbose("delayed request to return after iframe completes");
                                var delayedRequest = $q.defer();
                                authService.acquireToken(resource).then(function (token) {
                                    ...
                                    authService.verbose('Error during lookup ' + errDesc + ", " + error + ", a28fb650-a300-4352-9fdb-19d6bece2258");
                ...
                responseError: function (rejection) {
                    ...
                    if (rejection) {
                        if (rejection.status === 401) {
                            ...
                            authService("$broadcast, adal:notAuthorized 260d7f19-2e56-4e77-a9f7-7901e7e249c3");
                            $rootScope.$broadcast('adal:notAuthorized', rejection, resource);
                        }
                        else {
                            authService("$broadcast, adal:errorResponse 1755d23f-7815-4e11-b9ea-7f9e4fafbf75");
